-- CRITICAL SECURITY FIX: Simplify Payment Data Access Policies (Fixed v2)

-- 1. Drop existing function first to avoid return type conflict
DROP FUNCTION IF EXISTS public.get_vendor_payments_secure(uuid);

-- 2. Drop the complex policy that uses nested EXISTS clauses
DROP POLICY IF EXISTS "Payments ultra strict vendor isolation" ON public.payments;

-- 3. Create a secure function to determine payment access authorization
...
-- 4. Create simple, secure RLS policy using the security definer function
CREATE POLICY "Payments simple secure access" 
  ON public.payments 
  FOR SELECT 
  USING (can_access_payment_data(payments));

-- 5. Create additional policies for other operations with strict controls
...
-- 6. Create enhanced secure payment query function with proper validation
CREATE FUNCTION public.get_vendor_payments_secure(vendor_id_param uuid)
 RETURNS TABLE(
   id uuid,
   booking_id uuid,
   amount numeric,
...
INSERT INTO public.security_events (
  ...
  jsonb_build_object(
    'changes_applied', ARRAY[
      ...
      'eliminated_potential_logic_flaws'
    ],
    'security_improvement', 'replaced_complex_joins_with_secure_function',
    'timestamp', now()
  ),
  'critical',
